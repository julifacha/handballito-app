...
public partial class MatchService : IMatchService
{
    ...
    public async Task<MatchDto> AddMatchAsync(CreateMatchDto dto)
    {
        ...
        var existingMatch = await _db.Matches.AnyAsync(m => m.Date == dto.Date);
        if (existingMatch)
            throw new ArgumentException($"A match already exists for {dto.Date:dd/MM/yyyy}.");
    ...
    private async Task<CreateMatchFromImageResultDto> CreateMatchFromExtractedDataAsync(ExtractedMatchData extractedData)
    {
        ...
        var existingMatch = await _db.Matches.AnyAsync(m => m.Date == matchDate);
        if (existingMatch)
            throw new ArgumentException($"A match already exists for {matchDate:dd/MM/yyyy}.");
